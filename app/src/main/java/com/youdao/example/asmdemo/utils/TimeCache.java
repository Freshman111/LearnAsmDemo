package com.youdao.example.asmdemo.utils;
...
public class TimeCache {
    ...
    public static void printlnTime(String methodName, String className) {
        ...
        if (!mStartTimes.containsKey(key) || !mEndTimes.containsKey(key)) {
            System.out.println("TimeCache className =" + key + "not exist");
        }
        long currTime = mEndTimes.get(key) - mStartTimes.get(key);
        System.out.println("TimeCache className =" + className + " methodName =" + methodName + "，time consuming " + currTime + "  ms");
    }
}